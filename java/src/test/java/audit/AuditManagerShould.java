...
class AuditManagerShould {
    ...
    @Test
    void addsNewVisitorToExistingFileWhenMaxNumberOfEntriesIsNotYetReached() {
        IFileSystem fileSystemMock = Mockito.mock(IFileSystem.class);
        when(fileSystemMock.getFiles("audits"))
            .thenReturn(new String[] {"audits/audit_1.txt"});
        when(fileSystemMock.readAllLines("audits/audit_1.txt"))
            .thenReturn(List.of(
                "Peter;2019-04-06 16:30:00"
            ));

        var sut = new AuditManager(2, "audits", fileSystemMock);

        sut.addRecord("Alice", LocalDateTime.parse("2019-04-06T18:00:00"));

        verify(fileSystemMock).writeAllText("audits/audit_1.txt", "Alice;2019-04-06 18:00:00");
    }
